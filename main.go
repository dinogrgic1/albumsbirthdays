...
func loadYearAlbums(year int, verbose bool) map[string][]string {
	printMessage("Loading albums by year...", verbose)
	myTags := make(map[string][]string)
	myTagsCollector := colly.NewCollector()

	myTagsCollector.OnHTML("div.albumBlock", func(tagBlock *colly.HTMLElement) {
		printMessage("Loading albums by year...", verbose)
		tagBlock.ForEach(("a[href]"), func(_ int, linkElement *colly.HTMLElement) {
			myTagLink := linkElement.Attr("href")
			myTag := linkElement.Text
			albumIDs := getAlbumsIDForTag(year, myTag, myTagLink, verbose)
	...
	myTagsCollector.OnResponse(func(response *colly.Response) {
		printMessage("got body", verbose)
	})

	myTagsCollector.OnError(func(r *colly.Response, e error) {
		log.Println("error:", e, r.Request.URL, string(r.Body))
	})

	myTagsURI := fmt.Sprintf("https://www.albumoftheyear.org/%d/releases/", year)
	myTagsCollector.Visit(myTagsURI)

	return myTags
}

func getAlbumsIDForTag(year int, myTag string, myTagLink string, verbose bool) []string {
	...
	app := &cli.App{
		...
		Flags: []cli.Flag{
			&cli.IntFlag{
				...
				Aliases:     []string{"y"},
				Required:    true,
				Usage:       "year for export",
				Destination: &yearToExport,
			},
			&cli.BoolFlag{
				Name:        "verbose",
				Aliases:     []string{"v"},
		...
		Action: func(c *cli.Context) error {
			//myTags := make(map[string][]string)
			_ = loadYearAlbums(yearToExport, verbose)
			return nil
		},
	}
